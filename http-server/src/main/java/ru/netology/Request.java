...
public class Request {
    ...
    List<FileItem> clearFileItem = new ArrayList<>();
    String safeDirectory = "A:\\HomeWorkNetology\\" +
            "HTTP and the Modern Web\\jspr-code-master" +
            "\\jspr-code-master\\01_web\\FileLoadFromHttp";
    ...
    Request addRequest(HttpServletRequest httpServletRequest) throws IOException, FileUploadException {
        // new initialization
        DiskFileItemFactory factory = new DiskFileItemFactory();
        ServletFileUpload upload = new ServletFileUpload(factory);

        List<FileItem> fileItem  = upload.parseRequest(httpServletRequest);
        ...
        while (iter.hasNext()) {
            ...
            if (item.isFormField()) {
                clearFileItem.add(item);

            } else if (!item.isFormField()) {
                try {
                    File uploadedFile = new File(safeDirectory+"\\"+item.getName());
...
//    }
//
    FileItem getPart(String name) {
        Optional<FileItem> fileItem1 = clearFileItem.stream().filter(x->x.getName().equals(name)).findFirst();
        if (fileItem1.isPresent()) {
            return fileItem1.get();
        } else return null;
    }

    List<FileItem> getParts() {
        return clearFileItem;
    }
}